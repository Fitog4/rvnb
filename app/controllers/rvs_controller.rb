class RvsController < ApplicationController
  ...
  def filter_by_date
    if rv_search_params[:available_till].present?
      @rvs = Rv.near(params[:searched_location], 500).where('available_till >= ?', rv_search_params[:available_till])
    end

    if rv_search_params[:available_from].present?
      @rvs = Rv.near(params[:searched_location], 500).where('available_from <= ?', rv_search_params[:available_from])
    end
    render :search
  end
